...
int main() {
  ...
  // insertion
  printf("\n");
  printf("Inserting values 99, 999 and 42 on this list:\n");
  insert(first, 0, 99);
  display(first);
  insert(first, 4, 999);
  display(first);
  ...
  // make a new sorted ll
  printf("New ll B =\n");
  int B[] = {0, 10, 20, 30, 40, 50, 60, 70};
  create(B, 8);
  printf("\n");
  display(first);  

  // insert on a sorted ll
  printf("Inserting 35 and 5 on a sorted list B:\n");
  insertOnSorted(first, 35);
  printf("\n");
  display(first);
